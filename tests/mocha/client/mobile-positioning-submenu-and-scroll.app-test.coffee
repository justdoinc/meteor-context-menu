...
describe "Meteor Context Menu Scroll Binding", ->
  ...
  it "should support manual rebinding through bindScrollContainer and safely no-op for missing targets", ->
    target = createTarget
      left: 26
      top: 8
      width: 66

    $old_scroll_container = createScrollContainer scrollLeft: 0
    $new_scroll_container = createScrollContainer
      left: 0
      top: 50
      scrollLeft: 0

    {menuId, $menu} = openMenu
      target: target
      pageX: 92
      pageY: 54
      menuWidth: 78
    ...
    context.bindScrollContainer menuId, $old_scroll_container, -> true
    initial_left = parseFloat $menu.css "left"
    $old_scroll_container.scrollLeft 20
    $old_scroll_container.trigger "scroll"
    expect(parseFloat($menu.css("left"))).to.equal initial_left - 20

    context.bindScrollContainer menuId, $new_scroll_container, -> true
    left_before_old_scroll = parseFloat $menu.css "left"
    $old_scroll_container.scrollLeft 35
    $old_scroll_container.trigger "scroll"
    expect(parseFloat($menu.css("left"))).to.equal left_before_old_scroll

    $new_scroll_container.scrollLeft 15
    $new_scroll_container.trigger "scroll"
    expect(parseFloat($menu.css("left"))).to.equal left_before_old_scroll - 15

    noop_bind = ->
      context.bindScrollContainer "missing-menu", $(), ->
        return true
      return

    expect(noop_bind).not.to.throw()
    return
