	...
	function initialize(opts) {
		...
		$(document).on('mouseenter', '.dropdown-submenu', function(){
			var $sub = $(this).find('.dropdown-context-sub:first'),
				...
				isRtl = (typeof APP !== "undefined" && APP !== null ? (APP.justdo_i18n != null ? APP.justdo_i18n.isRtl() : void 0) : void 0);
			
			if (isRtl) {
				// RTL mode: submenus open to the left by default
				// Check for collision with left edge of screen
				var collisionLeft = subLeft - subWidth < 0;
				if (collisionLeft) {
					// Not enough room on left, open to right instead
					$sub.removeClass('drop-left');
				} else {
					// Default RTL behavior: open to left
					$sub.addClass('drop-left');
				}
			} else {
				// LTR mode: submenus open to the right by default
				// Check for collision with right edge of screen
				var collisionRight = (subWidth + subLeft) > window.innerWidth;
				if (collisionRight) {
					// Not enough room on right, open to left instead
					$sub.addClass('drop-left');
				}
			}
		});
        ...
        $(selector).on('contextmenu', function (e) {
            ...
            if (typeof options.above == 'boolean' && options.above) {
                ...
                if ((e.pageY + autoH) > $('html').height()) {
                    $dd.addClass('dropdown-context-up').css({
                        top: e.pageY - CONSTANTS.VERTICAL_OFFSET_ABOVE - autoH,
                        left: left
                    }).fadeIn(options.fadeSpeed);
                } else {
                    $dd.css({
            ...
            if (typeof options.left == 'boolean' && options.left) {
                ...
                if ((e.pageX + autoL) > $('html').width()) {
                    $dd.addClass('dropdown-context-left').css({
                        left: e.pageX - $dd.width() + CONSTANTS.HORIZONTAL_OFFSET
                    });
                }
            }
        });
