...
context = (function () {
	...
	function initialize(opts) {
		...
		$(document).on('mouseenter', '.dropdown-submenu', function(){
			...
			// Use parent position + submenu width to detect overflow
			// (avoids needing to read submenu's rendered position)
			var parentRight = $this.offset().left + $this.outerWidth(),
				collision;

			if (rtl) {
				// RTL: submenus open LEFT by default, check left edge
				collision = ($this.offset().left - $sub.outerWidth()) < 0;
			} else {
				// LTR: submenus open RIGHT by default, check right edge
				collision = (parentRight + $sub.outerWidth()) > window.innerWidth;
			}
